...
# -*- coding: utf-8 -*-
#------------------------------------------------------------------------------
# Script: mise_a_niveau.sh
# Description: Synchronise le projet avec les dernières décisions
#   1. Crée src/generate_bash.sh (conversion XML → Bash avec xmllint)
#   2. Met à jour les constantes FR dans src/macrosTitux.py
...
#==============================================================================
# ÉTAPE 1: Créer src/generate_bash.sh
#==============================================================================
create_generate_bash() {
    log "Création de src/generate_bash.sh ..."

    cat > "$SRC_DIR/generate_bash.sh" << 'GEN_BASH_EOF'
#!/bin/bash
# -*- coding: utf-8 -*-
#------------------------------------------------------------------------------
...
GEN_BASH_EOF
    ...
    log "✓ generate_bash.sh créé"
}

#==============================================================================
# ÉTAPE 2: Mettre à jour src/macrosTitux.py
#==============================================================================
update_python_constants() {
    log "Mise à jour des constantes dans src/macrosTitux.py ..."

    cat > "$SRC_DIR/constants_update.patch" << 'PATCH_EOF'
--- constantes_anciennes ---
TRIGGERS = {
    "CRON": "Cron (planifie)",
...
PATCH_EOF
    ...
    # Appliquer les changements directement avec sed
    sed -i 's/"CRON": "Cron (planifie)"/"DEMARRAGE": "Au démarrage"/' "$SRC_DIR/macrosTitux.py"
    sed -i 's/"ON_BOOT": "Au demarrage"/"HORAIRE": "Horaire"/' "$SRC_DIR/macrosTitux.py"
    sed -i 's/"FICHIER_MODIFIE": "Fichier modifie"/"FICHIER_MODIFIE": "Fichier modifié"/' "$SRC_DIR/macrosTitux.py"
    sed -i 's/"FICHIER_CREE": "Fichier cree"/"FICHIER_CREE": "Fichier créé"/' "$SRC_DIR/macrosTitux.py"
    sed -i 's/"USB_CONNECT": "USB connecte"/"USB_CONNECTE": "USB connecté"/' "$SRC_DIR/macrosTitux.py"
    sed -i 's/"INTERFACE_RESEAU": "Interface reseau"/"RESEAU_ACTIF": "Réseau activé",\n    "SORTIE_TUBE": "Sortie de tube"/' "$SRC_DIR/macrosTitux.py"

    sed -i 's/"COPY_FILE": "Copier fichier(s)"/"COPIER_FICHIER": "Copier fichier(s)"/' "$SRC_DIR/macrosTitux.py"
    sed -i 's/"MOVE_FILE": "Deplacer fichier(s)"/"DEPLACER_FICHIER": "Déplacer fichier(s)"/' "$SRC_DIR/macrosTitux.py"
    sed -i 's/"DELETE_FILE": "Supprimer fichier(s)"/"SUPPRIMER_FICHIER": "Supprimer fichier(s)"/' "$SRC_DIR/macrosTitux.py"
    sed -i 's/"NOTIFY": "Envoyer notification"/"NOTIFIER": "Notifier"/' "$SRC_DIR/macrosTitux.py"
    sed -i 's/"RUN_CMD": "Executer commande"/"EXECUTER_CMD": "Exécuter commande"/' "$SRC_DIR/macrosTitux.py"
    sed -i 's/"RESTART_SVC": "Redemarrer service"/"REDÉMARRER_SERV": "Redémarrer service",\n    "SORTIR_RESULTAT": "Sortir résultat dans tube"/' "$SRC_DIR/macrosTitux.py"

    sed -i 's/"DISK_SPACE": "Espace disque"/"ESPACE_DISQUE": "Espace disque disponible"/' "$SRC_DIR/macrosTitux.py"
    sed -i 's/"TIME_RANGE": "Plage horaire"/"PLAGE_HORAIRE": "Plage horaire"/' "$SRC_DIR/macrosTitux.py"
    sed -i 's/"PROC_RUNNING": "Processus actif"/"PROCESSUS_ACTIF": "Processus actif"/' "$SRC_DIR/macrosTitux.py"

    log "✓ Constantes mises à jour"
}

#==============================================================================
# ÉTAPE 3: Résumé et instructions
#==============================================================================
show_summary() {
    log ""
    log "=========================================="
    log "MISE À NIVEAU TERMINÉE"
    log "=========================================="
    log ""
    log "Fichiers créés/modifiés :"
    echo "  • src/generate_bash.sh  (nouveau)"
    echo "  • src/macrosTitux.py    (modifié - constantes FR)"
    log ""
    log "Pour valider Git :"
    echo "  git add ."
    echo "  git commit -m \"Mise à niveau: génération Bash + constantes FR\""
    echo "  git push"
    log ""
    log "Pour tester :"
    echo "  cat > /tmp/test.xml << 'EOF'"
    echo '<?xml version="1.0" encoding="UTF-8"?>'
    echo '<macro name="Test">'
    echo '    <trigger type="DEMARRAGE" label="Au démarrage"/>'
    echo '    <actions>'
    echo '        <action id="1" type="NOTIFIER">'
    echo '            <param key="titre" value="Test"/>'
    echo '            <param key="message" value="Marche !"/>'
    echo '        </action>'
    echo '    </actions>'
    echo '</macro>'
    echo 'EOF'
    echo "  ./src/generate_bash.sh /tmp/test.xml /tmp/test.sh"
    echo "  /tmp/test.sh"
    log "=========================================="
}
...
#==============================================================================
main() {
    log "Démarrage de la mise à niveau..."
